...
def footprint(el):
    """Footprint polygon for an OSM way or multipolygon relation, or None."""
    lines = []
    if el["type"] == "way":
        lines.append(el.get("geometry", []))
    elif el["type"] == "relation":
        # Outer rings can be split across several member ways, so merge them before polygonizing.
        lines += [m["geometry"] for m in el.get("members", []) if m.get("role") in ("outer", "") and m.get("geometry")]
    lines = [LineString([proj(p["lon"], p["lat"]) for p in ring]) for ring in lines if len(ring) >= 2]
    if not lines: return None
    try:
        polys = [p.buffer(0) for p in polygonize(unary_union(linemerge(lines)))]
    except Exception:
        return None
